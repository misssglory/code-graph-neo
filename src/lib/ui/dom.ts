export function createDom() {
  return {
    appRoot: document.getElementById('app-root'),
    graphContainer: document.getElementById('graph-container'),
    appTitle: document.getElementById('app-title'),
    inspect: document.getElementById('inspect'),
    selection: document.getElementById('selection'),
    search: document.getElementById('search'),
    searchHints: document.getElementById('search-hints'),
    searchMatchName: document.getElementById('search-match-name'),
    searchMatchFilename: document.getElementById('search-match-filename'),
    searchMatchCode: document.getElementById('search-match-code'),
    summaryType: document.getElementById('summary-type'),
    summaryMain: document.getElementById('summary-main'),
    summaryNodes: document.getElementById('summary-nodes'),
    summaryFiles: document.getElementById('summary-files'),
    graphSnapshotRefreshBtn: document.getElementById('graph-snapshot-refresh'),
    graphSnapshotStatus: document.getElementById('graph-snapshot-status'),
    graphSnapshotList: document.getElementById('graph-snapshot-list'),
    graphSnapshotSort: document.getElementById('graph-snapshot-sort'),
    graphSnapshotDetails: document.getElementById('graph-snapshot-details'),

    searchHintsOverlay: document.getElementById('search-hints-overlay'),
    searchAddToStateBtn: document.getElementById('search-add-to-state'),
    searchAddFocusedBtn: document.getElementById('search-add-focused'),
    selectedAddNodeBtn: document.getElementById('selected-add-node'),
    selectedAddIncomingBtn: document.getElementById('selected-add-incoming'),
    selectedAddOutgoingBtn: document.getElementById('selected-add-outgoing'),
    selectedRemoveIncomingBtn: document.getElementById('selected-remove-incoming'),
    selectedRemoveOutgoingBtn: document.getElementById('selected-remove-outgoing'),
    selectedAddPathBtn: document.getElementById('selected-add-path'),
    selectedRemovePathBtn: document.getElementById('selected-remove-path'),
    selectedClearAllBtn: document.getElementById('selected-clear-all'),
    selectedMutationHints: document.getElementById('selected-mutation-hints'),
    subgraphDirection: document.getElementById('subgraph-direction'),
    subgraphMaxDepth: document.getElementById('subgraph-max-depth'),
    subgraphStatus: document.getElementById('subgraph-status'),
    subgraphDepthList: document.getElementById('subgraph-depth-list'),
    subgraphDetail: document.getElementById('subgraph-detail'),
    selectionHistoryStatus: document.getElementById('selection-history-status'),
    selectionHistoryList: document.getElementById('selection-history-list'),
    bulkTextInput: document.getElementById('bulk-text-input'),
    bulkRenderMarkdown: document.getElementById('bulk-render-markdown'),
    bulkNameMatchMode: document.getElementById('bulk-name-match-mode'),
    bulkFilenameMatchMode: document.getElementById('bulk-filename-match-mode'),
    bulkAnnotatedText: document.getElementById('bulk-annotated-text'),
    bulkAddBtn: document.getElementById('bulk-add'),
    bulkRemoveBtn: document.getElementById('bulk-remove'),
    bulkStatus: document.getElementById('bulk-status'),
    bulkProgress: document.getElementById('bulk-progress'),
    bulkProgressBar: document.getElementById('bulk-progress-bar'),
    bulkProgressLabel: document.getElementById('bulk-progress-label'),
    bulkMatchAnnotations: document.getElementById('bulk-match-annotations'),
    bulkAddHints: document.getElementById('bulk-add-hints'),
    bulkRemoveHints: document.getElementById('bulk-remove-hints'),
    insertFunctionInput: document.getElementById('insert-function-input'),
    insertFunctionStatus: document.getElementById('insert-function-status'),
    insertFunctionSubtabs: document.getElementById('insert-function-subtabs'),
    insertFunctionMatches: document.getElementById('insert-function-matches'),
    insertFunctionCodeView: document.getElementById('insert-function-code-view'),
    selectedCopyBtn: document.getElementById('selected-copy'),
    selectedStatus: document.getElementById('selected-status'),
    selectedList: document.getElementById('selected-list'),
    selectedCodeView: document.getElementById('selected-code-view'),
    pathFromInput: document.getElementById('path-from'),
    pathToInput: document.getElementById('path-to'),
    pathGoBtn: document.getElementById('path-go'),
    pathClearBtn: document.getElementById('path-clear'),
    pathReverseBtn: document.getElementById('path-reverse'),
    pathStatus: document.getElementById('path-status'),
    pathSelectionSummary: document.getElementById('path-selection-summary'),
    pathFoundList: document.getElementById('path-found-list'),
    pathList: document.getElementById('path-list'),
    pathCodeView: document.getElementById('path-code-view'),
    pathCopyBtn: document.getElementById('path-copy-selected'),
    directedToggle: document.getElementById('directed-toggle'),
    renderEdgeDirectionToggle: document.getElementById('render-edge-direction-toggle'),
    lineNumbersToggle: document.getElementById('line-numbers-toggle'),
    wordWrapToggle: document.getElementById('word-wrap-toggle'),
    layoutModeSelect: document.getElementById('layout-mode'),
    sizeModeSelect: document.getElementById('node-size-mode'),
    sizeBaseInput: document.getElementById('node-size-base'),
    sizeCodeFactorInput: document.getElementById('node-size-code-factor'),
    sizeBaseValue: document.getElementById('node-size-base-value'),
    sizeCodeFactorValue: document.getElementById('node-size-code-factor-value'),
    mainSourceSelect: document.getElementById('main-source-select'),
    recomputeMainBtn: document.getElementById('recompute-main'),
    transparencyInput: document.getElementById('pane-transparency'),
    transparencyValue: document.getElementById('pane-transparency-value'),
    collapseSidebarBtn: document.getElementById('collapse-sidebar'),
    mainComponentFocusBtn: document.getElementById('main-component-focus'),
    sidebarTabs: document.querySelectorAll('[data-tab-button]'),
    sidebarPanels: document.querySelectorAll('[data-tab-panel]'),
    rightPane: document.getElementById('right-pane'),
    rightPaneWrap: document.getElementById('right-pane-wrap'),
    rightPaneResizeCorner: document.getElementById('right-pane-resize-corner'),
  };
}
